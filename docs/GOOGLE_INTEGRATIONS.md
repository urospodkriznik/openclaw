...
2. **gogcli data for Docker (important):** the gateway runs as **UID 1000** (`node`). If **`OPENCLAW_GOGCLI_CONFIG_DIR`** points at **`~/.config/gogcli`** owned only by your SSH user, **`gog`** inside the container gets **`permission denied`**. Recommended:
   ...
   - After every host **`gog auth`** (or token refresh), run **`./scripts/sync-gog-cli-config.sh`** (or **`make sync-gog-config`**) from repo root: it **`rsync`**s as your user (no **`sudo rsync`**), then **`sudo -n chown …`** to **`1000:1000`**. That needs the **same passwordless `chown`** sudoers entry as **`./scripts/reown-openclaw-mounts.sh`** (see **`docs/GITHUB_ACTIONS.md`**).
