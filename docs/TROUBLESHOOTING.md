...
**Fix:** use **`OPENCLAW_GOGCLI_CONFIG_DIR=./.openclaw-gog-config`**, run **`./scripts/sync-gog-cli-config.sh`** after host **`gog auth`** (copies **`~/.config/gogcli`** into that dir and **`chown`s to `1000:1000`**), then **`./scripts/docker-compose.sh restart openclaw-gateway`**. See **[docs/GOOGLE_INTEGRATIONS.md](GOOGLE_INTEGRATIONS.md)** (gog skill).
